...
@section('meta_description', 'Téléchargez le sujet de ' . $sujet->matiere->libelle . ' : ' . $sujet->titre . '. Document
    éducatif gratuit avec corrigé disponible.')
@section('meta_keywords', $sujet->matiere->libelle . ', sujet gratuit, exercice corrigé, téléchargement, ' .
    $sujet->titre)
@section('og_title', $sujet->titre . ' - Sujet de ' . $sujet->matiere->libelle)
@section('og_description', 'Téléchargez gratuitement ce sujet de ' . $sujet->matiere->libelle . ' avec corrigé
    détaillé.')
...
@section('content')
                        ...
                        <div class="preview-section mb-3">
                                    ...
                                    <div class="preview-container" data-bs-toggle="modal" data-bs-target="#previewModal"
                                        ...
                                        @if ($isPdf)
                                            <iframe src="{{ $fileUrl }}#toolbar=0&navpanes=0&scrollbar=0"
                                                class="pdf-preview" title="Aperçu du sujet"></iframe>
                                        ...
                                        @elseif($isDoc)
                                            <iframe
                                                ...
                                                class="pdf-preview" title="Aperçu du document"></iframe>
                                            <div class="file-info-badge">
                                                <i
                                                    class="bi bi-filetype-{{ strtolower($fileExtension) }} me-1"></i>{{ strtoupper($fileExtension) }}
                                                • {{ $fileSizeMB }}MB
                                            </div>
                                        @else
                                            <div class="d-flex align-items-center justify-content-center"
                                                style="height: 200px; background: #f8f9fa; border-radius: 8px;">
                                                <div class="text-center">
                                                    <i class="bi bi-file-earmark text-muted" style="font-size: 3rem;"></i>
                                                    <p class="mt-2 text-muted">{{ $fileName }}</p>
                                                    <small class="text-muted">{{ strtoupper($fileExtension) }} •
                                                        {{ $fileSizeMB }}MB</small>
                                                </div>
                                            </div>
                                        @endif
                                ...
                                @else
                                    <div class="alert alert-simple alert-info">
                                        @if (auth()->user()->points <= 0)
                                            ...
                                            Points insuffisants pour voir l'aperçu et télécharger
                                        @else
                            ...
                            @else
                                <div class="alert alert-simple alert-warning">
                                    ...
                                    Connectez-vous pour voir l'aperçu et télécharger
                            ...
                            @if (auth()->user()->points > 0 && $sujet->getFirstMediaUrl('non_corrige'))
                                <div class="d-grid gap-2">
                                    ...
                                    <button type="button" class="btn btn-outline-primary w-100" data-bs-toggle="modal"
                                        data-bs-target="#previewModal" data-file-url="{{ $fileUrl }}"
                                        data-file-type="{{ $fileExtension }}" data-file-name="{{ $fileName }}">
                                    ...
                                    <div class="preview-container" data-bs-toggle="modal" data-bs-target="#corrigePreviewModal"
                                        ...
                                        @if ($isPdfCorrige)
                                            <iframe src="{{ $corrigeUrl }}#toolbar=0&navpanes=0&scrollbar=0"
                                                class="pdf-preview" title="Aperçu du corrigé"></iframe>
                                            <div class="file-info-badge">
                                                <i class="bi bi-filetype-pdf me-1"></i>PDF • {{ $corrigeSizeMB }}MB
                                            </div>
                                        @elseif($isDocCorrige)
                                            <iframe
                                                src="https://view.officeapps.live.com/op/embed.aspx?src={{ urlencode($corrigeUrl) }}"
                                                class="pdf-preview" title="Aperçu du corrigé"></iframe>
                                            <div class="file-info-badge">
                                                <i
                                                    class="bi bi-filetype-{{ strtolower($corrigeExtension) }} me-1"></i>{{ strtoupper($corrigeExtension) }}
                                                • {{ $corrigeSizeMB }}MB
                                            </div>
                                        @else
                                            <div class="d-flex align-items-center justify-content-center"
                                                style="height: 200px; background: #f8f9fa; border-radius: 8px;">
                                                <div class="text-center">
                                                    <i class="bi bi-file-earmark-check text-success"
                                                        style="font-size: 3rem;"></i>
                                                    <p class="mt-2 text-muted">{{ $corrigeName }}</p>
                                                    <small class="text-muted">{{ strtoupper($corrigeExtension) }} •
                                                        {{ $corrigeSizeMB }}MB</small>
                                                </div>
                                            </div>
                                        @endif
                                        <div class="preview-overlay">
                            ...
                            @if (auth()->user()->points > 0 && $sujet->getFirstMediaUrl('corrige'))
                                <div class="d-grid gap-2">
                                    ...
                                    <button type="button" class="btn btn-outline-success w-100" data-bs-toggle="modal"
                                        data-bs-target="#corrigePreviewModal" data-file-url="{{ $corrigeUrl }}"
                                        data-file-type="{{ $corrigeExtension }}" data-file-name="{{ $corrigeName }}">
                                        <i class="bi bi-eye me-2"></i>
                                        Aperçu avant télécharger
                                    </button>
                                </div>
    ...
    <div class="modal fade preview-modal" id="corrigePreviewModal" tabindex="-1"
        aria-labelledby="corrigePreviewModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (previewModal) {
                previewModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    if (fileType.toLowerCase() === 'pdf') {
                        modalBody.innerHTML =
                            `<iframe src="${fileUrl}#toolbar=1&navpanes=1&scrollbar=1" class="full-preview" title="Aperçu complet"></iframe>`;
                    } else if (['doc', 'docx'].includes(fileType.toLowerCase())) {
                        modalBody.innerHTML =
                            `<iframe src="https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(fileUrl)}" class="full-preview" title="Aperçu complet"></iframe>`;
                    } else {
                        modalBody.innerHTML = `
            ...
            if (corrigePreviewModal) {
                corrigePreviewModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    if (fileType.toLowerCase() === 'pdf') {
                        modalBody.innerHTML =
                            `<iframe src="${fileUrl}#toolbar=1&navpanes=1&scrollbar=1" class="full-preview" title="Aperçu complet du corrigé"></iframe>`;
                    } else if (['doc', 'docx'].includes(fileType.toLowerCase())) {
                        modalBody.innerHTML =
                            `<iframe src="https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(fileUrl)}" class="full-preview" title="Aperçu complet du corrigé"></iframe>`;
                    } else {
                        modalBody.innerHTML = `
                    <div class="text-center py-5">
